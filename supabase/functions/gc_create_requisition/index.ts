...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const euaRes = await gcFetchRaw(env, tokenHolder, "agreements/enduser/", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        max_historical_days: 365, access_valid_for_days: 180,
        access_scope: ["balances", "details", "transactions"],
      }),
    }, correlationId, env.TIMEOUT_EUA_MS!, refresh);

    if (!euaRes.ok) {
      const t = await euaRes.text().catch(() => "");
      return new Response(JSON.stringify({
        error: "Bad Gateway", code: "UPSTREAM_EUA_ERROR", correlationId,
    ...
    // Create requisition
    const rqRes = await gcFetchRaw(env, tokenHolder, "requisitions/", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (!requisitionId || !link) {
      return new Response(JSON.stringify({ error: "Bad Gateway", code: "UPSTREAM_REQUISITION_MALFORMED", correlationId }), {
        status: 502, headers: { "content-type": "application/json", ...allow(req) },
      });
    }

    // DB: ensure a connected_banks row (use requisitionId as placeholder account_id to satisfy unique(user_id,account_id))
    ...
    if (cbErr || !cbRow?.id) {
      console.error("gc_create_requisition db upsert failed", { correlationId, error: cbErr?.message });
    ...
    return new Response(JSON.stringify({
      link, requisition_id: requisitionId, connected_bank_id: cbRow.id,
    ...
    return new Response(JSON.stringify({
      error: "Internal Server Error", code: "GC_CREATE_REQUISITION_FAILURE", correlationId,
